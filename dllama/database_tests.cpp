...
namespace {
	...
	TEST_F(DatabaseTest, GoogleTestTest) {
		EXPECT_EQ(0, 3 - 3);
		EXPECT_NE(0, 2);
		EXPECT_TRUE(1==1);
	}
	
	void add_nodes_test(graph_database* dllama_instance) {
		dllama_instance->load_net_graph("simple_graph.net");
			
		if (world_rank == 0) {
			dllama_instance->add_nodes(10);
		}

		MPI_Barrier(MPI_COMM_WORLD);
		EXPECT_EQ(dllama_instance->max_nodes(), 14);
		MPI_Barrier(MPI_COMM_WORLD);
	...
	TEST_F(DatabaseTest, DLLAMAAddNodesTest) {
		MPI_Barrier(MPI_COMM_WORLD);
		dllama* dllama_instance = new dllama("database/", false);
		add_nodes_test(dllama_instance);
	}
	
	TEST_F(DatabaseTest, SimpleDLLAMAAddNodesTest) {
		MPI_Barrier(MPI_COMM_WORLD);
		simple_dllama* dllama_instance = new simple_dllama("database/", false);
		add_nodes_test(dllama_instance);
	}
	
	TEST_F(DatabaseTest, LLAMAAddNodesTest) {
		MPI_Barrier(MPI_COMM_WORLD);
		llama_for_benchmark* dllama_instance = new llama_for_benchmark("database/", false);
		add_nodes_test(dllama_instance);
	}
	
	void add_edges_test(graph_database* dllama_instance) {
		dllama_instance->load_net_graph("simple_graph.net");
		if (world_rank == 0) {
			...
			node_t* expected_neighbours[4] = {expected_neighbours1, expected_neighbours2, expected_neighbours3, expected_neighbours4};
			size_t expected_degree[4] = {3, 3, 3, 0};
	...
	TEST_F(DatabaseTest, DLLAMAAddEdgesTest) {
		MPI_Barrier(MPI_COMM_WORLD);
		dllama* dllama_instance = new dllama("database/", false);
		add_edges_test(dllama_instance);
	}
	
	TEST_F(DatabaseTest, SimpleDLLAMAAddEdgesTest) {
		MPI_Barrier(MPI_COMM_WORLD);
		simple_dllama* dllama_instance = new simple_dllama("database/", false);
		add_edges_test(dllama_instance);
	}
	
	TEST_F(DatabaseTest, LLAMAAddEdgesTest) {
		MPI_Barrier(MPI_COMM_WORLD);
		llama_for_benchmark* dllama_instance = new llama_for_benchmark("database/", false);
		add_edges_test(dllama_instance);
	}
